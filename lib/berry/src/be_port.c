...
/* Опційне захоплення виводу print() у буфер — для ендпоінта /script/run (повертаємо
   текстовий вивід скрипта назад у веб/чат). Керується ззовні be_capture_begin/end.
   Коли захоплення активне, дублюємо і в буфер, і на Serial. */
static char*  s_cap_buf = 0;
static size_t s_cap_cap = 0;
static size_t s_cap_len = 0;
void be_capture_begin(char *buf, size_t cap) { s_cap_buf = buf; s_cap_cap = cap; s_cap_len = 0; if (cap) buf[0] = 0; }
size_t be_capture_end(void) { size_t n = s_cap_len; s_cap_buf = 0; s_cap_cap = 0; s_cap_len = 0; return n; }

BERRY_API void be_writebuffer(const char *buffer, size_t length)
{
    if (s_cap_buf && s_cap_cap) {
        size_t i;
        for (i = 0; i < length && s_cap_len < s_cap_cap - 1; i++)
            s_cap_buf[s_cap_len++] = buffer[i];
